...
{\section{Introducere}
Un glob de plasmă este un recipient din sticlă transparentă umplut cu un amestec de diferite gaze nobile, cu un electrod de înaltă tensiune în centru.

Atunci când aplicăm tensiune, se formează o plasmă în interiorul recipientului. Filamentele de plasmă se extind de la electrodul interior până la izolatorul exterior din sticlă, sub forma unor fascicule colorate diferit în funcție de gazul nobil folosit.

Globul de plasmă a fost inventat de Nikola Tesla, în timpul experimentelor pe care le făcea cu curenți de  frecvență mare într-un tub de sticlă vidat. Tesla l-a numit "tub de descărcare în gaz inert", dar versiunea modernă, cunoscută de toată lumea, a fost dezvoltat de James Falk și Bill Parker.
...
Tensiunea de alimentare de 5V (care provine de la un cablu USB sau de la trei baterii de tip AAA) este atenuată de capacitorul "C". Partea imprejmuită cu roșu alcatuiește un circuit oscilant, al cărui frecvență de oscilație este dictată de frecvența de oscilație a inductorului numit "Reacție", acesta, la rândul lui, având aceeși frecvență cu frecvența proprie a inductorului numit "Primar". Între inductorii "Primar" și "Secundar" are loc fenomenul de inducție electromagnetică. Astfel, în bucla "Secundar" regăsim o tensiune de câțiva mii de volți la o frecvență de până la câțiva zeci de kilohertzi.
...
Pentru a putea descrie potențialul electric de pe suprafața globului de plasmă vom proceda astfel:
...
Pasul 4: Se repetă măsuratorile de 10 ori, iar media măsurătoriilor se trece intr-un tabel.
...
În urma măsurătorilor a rezultat tabelul de valori \ref{tab1} și figura \ref{fig:ex}.
Se observă că odată cu coborârea pe globul de plasmă, diferența de potențial tinde să crească. Ne așteptam la acest femonem deoarece distanța dintre electrodul de sticlă și proba multimetrului crește, deci și diferența de potențial dintre masă și punctul în care se face măsurătoarea crește.

O mențiune importantă este că în distribuția potențialului pe suprafața sferei de sticlă există o asimetrie datorată poziționării masei aproape de punctul jos față (lucru vizibil în figuriile \ref{pct} și ref{princ}). Astfel la poziționarea brațului de măsură al multimetrului pare că fasciculul de plasmă "se scurge" spre masă (vezi figura \ref{scr}).
...
{\section{Concluzii}
În urma analizei modului de funcționare al unui glob de plasmă am descris modul în care se crează o tensiune de frecvență și voltaj mare pe electrodul de sticlă dinauntrul globului de plasmă umplut cu un gaz inert. Am aflat distribuția potențialului pe suprafața sferei de sticlă prin interpolarea valorilor măsurate în diverse puncte. Am arătat faptul că există o asimetrie în distribuția de potential datorată poziționării masei dinauntrul globului de sticlă.
}
